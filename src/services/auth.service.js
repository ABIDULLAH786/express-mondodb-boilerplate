...
module.exports.loginUserWithEmailAndPassword = async (email, password) => {
    ...
    if (!user || !(await user?.isPasswordMatch(password))) {
        throw new ApiError('Incorrect email or password', HTTP_STATUS_CODES.UNAUTHORIZED);
    }
    return user;
};
...
module.exports.logout = async (refreshToken) => {
    ...
    if (!refreshTokenDoc) {
        throw new ApiError('Not found', HTTP_STATUS_CODES.NOT_FOUND);
    }
...
module.exports.refreshAuth = async (refreshToken) => {
    try {
        ...
        throw new ApiError('Please authenticate', HTTP_STATUS_CODES.UNAUTHORIZED);
    }
};
...
module.exports.resetPassword = async (resetPasswordToken, newPassword) => {
    try {
        ...
        throw new ApiError('Password reset failed', HTTP_STATUS_CODES.UNAUTHORIZED);
    }
};
...
module.exports.verifyEmail = async (verifyEmailToken) => {
    ...
    if (verified) {
        ...
        throw new ApiError('Email verification failed', HTTP_STATUS_CODES.BAD_REQUEST);
    }
};
